...
void printtt(void){
   ...
   while(1){
      ...
      set_cursor(row++, col);
      print_string("thread name: ");
      print_string(memBegin -> threads[3].tName);
      set_cursor(row++, col);
      print_string("thread state: ");
      printState(memBegin -> threads[3].curState);
      sei();
...
void debug(void){
   set_cursor(15, 2);
   print_string("Before mutex lock: ");
   print_int(printLock->lock);
   print_string(" ");
   printState(memBegin -> threads[memBegin -> runningThread].curState);
   mutex_lock(printLock);

   set_cursor(17, 2);
   print_string("In critical area:");
   print_int(printLock->lock);
   print_string(" ");
   printState(memBegin -> threads[memBegin -> runningThread].curState);
...
void mutex_lock(struct mutex_t* m){
   ...
   if(m -> lock < 1){ //lock unavailable
      ...
      // print_string("Current state in mutex_lock: ");
      // printState(memBegin -> threads[memBegin -> runningThread].curState);
      memBegin -> threads[memBegin -> runningThread].curState = THREAD_WAITING;
      // set_cursor(20, 40 + 30);
      // printState(memBegin -> threads[memBegin -> runningThread].curState);
      ...
      // set_cursor(30, 2);
      // print_string("Lock being set for thread: ");
      // set_cursor(30, 30);
      // print_string(memBegin -> threads[memBegin -> runningThread].tName);
